...
//This line iterates through each key-value pair of the JSON object. Line 59                	
                    sb.append(entry.getKey()).append(" { ");
//This line appends the current key to a StringBuilder, followed by an opening brace, 
//to start constructing a nested field. Line 61                    
                    JsonArray subFieldArray = entry.getValue().getAsJsonArray();
//This line retrieves the value associated with the current key, which should be a JsonArray of subfields. Line 64                   
                    for (JsonElement subFieldElement : subFieldArray) {
//This line iterates through each subfield in the subFieldArray. Line 66                   	
                        sb.append(subFieldElement.getAsString()).append(" ");
//This line appends each subfield to the StringBuilder and adds a space character after it. Line 68                       
                    }
                    sb.append("} ");
//This line appends a closing brace to the StringBuilder to complete the nested field.   Line 71                 
                }
                studentInformationList.add(sb.toString());
//This line adds the completed StringBuilder object to the studentInformationList.  Line 74              
            }
        }

        StringBuilder queryBuilder = new StringBuilder("query { "); 
//This line initializes a StringBuilder object with a query string that starts with "query { ". Line 79
        queryBuilder.append(queryName).append("(id: ").append(id).append(") { ");
//This line appends the query name and ID to the query string, 
//which will allow the GraphQL server to identify the specific query to execute. Line 81
        for (String field : studentInformationList) {
                  queryBuilder.append(field).append(" ");
        } //This line appends the list of fields to retrieve for the query. Line 84-85
        queryBuilder.append("} }"); // This line appends the closing curly brackets to the query string. Line 87
        String queryStr = queryBuilder.toString();
// This line converts the StringBuilder object to a string, which can be used as the GraphQL query. Line 88
        GraphQLRequest request = GraphQLRequest.builder().query(queryStr).build();
//This line creates a GraphQL request object using the query string. Line 90       
        GraphQLResponse graphQLResponse = graphQLWebClient.post(request).block();
//This line sends the GraphQL request to the server using a WebClient and blocks until a response is received. Line 92
        
/* The block() method used in line 92 is used to wait for the response to be received before 
